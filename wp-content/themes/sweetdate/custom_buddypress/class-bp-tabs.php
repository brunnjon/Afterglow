...
class BpMembersTabs {
    ...
    public function render()
    {
        ...
        foreach ($this->tabs as $tab)
        {
            ...
            {
                $active = '';
                if($this->active_tab == sanitize_title($tab['name']) ) {
                    $active = 'active';  
                }
                
                echo '<li '.$active.' id="'.sanitize_title($tab['name']).'Tab" class="'.$active." ".$tab['class'].'">';
...
class BpMembersTab
{
    ...
    public function title()
    {
        ...
        return '<dd class="'.$active.'"><a href="#'.sanitize_title($this->args["name"]).'">'.$this->args["name"].'</a></dd>';
    }
...
if (!class_exists('BpMembersTab_rt_media')):
	...
	class BpMembersTab_rt_media extends BpMembersTab 
	{
		...
		public function title() 
		{
			...
			return '<dd class="sliderEvent '.$active.'"><a href="#'.sanitize_title($this->args["name"]).'">'.$this->args["name"].' <span class="radius label alert">'.$this->number.'</span></a></dd>';
		}
      ...
      $where .= " AND {$table_name}.context = 'profile'";
			...
			if($myrows && count($myrows) > 0)
			{
				...
				foreach($myrows as $row)
				{
					...
					$this->tabs_instance->fields_data[$name] .= '<span class="circle">';
					$this->tabs_instance->fields_data[$name] .= '<a href="' . trailingslashit( get_rtmedia_user_link( $displayed_user ) ) . RTMEDIA_MEDIA_SLUG . '/' . rtmedia_id( $row->media_id ) . '" class="imagelink">';
					$this->tabs_instance->fields_data[$name] .= '<span class="overlay"></span>';
					$this->tabs_instance->fields_data[$name] .= '<span class="read"><i class="icon-'. apply_filters('kleo_img_rounded_icon','heart').'"></i></span>';
