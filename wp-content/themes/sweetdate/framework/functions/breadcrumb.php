...
function breadcrumb_trail_get_buddypress_items($args = array()) 
{
	...
	if ( !empty( $bp->displayed_user->fullname ) ) { // looking at a user or self
		...
		$trail[] = '<a href="'. get_permalink() .'" title="'. ucwords( get_the_title() ) .'">'. ucwords( get_the_title() ) .'</a>';
		$trail[] = '<a href="'. get_permalink() . $bp->current_item .'" title="'.$bp->bp_options_title.'">'. $bp->bp_options_title .'</a>';		
