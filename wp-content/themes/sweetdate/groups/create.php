...
get_header( 'buddypress' ); ?>
    ...
    <?php get_template_part('page-parts/buddypress-before-wrap');?>
		...
		<form action="<?php bp_group_creation_form_action(); ?>" method="post" id="create-group-form" class="standard-form <?php if ( !bp_is_group_creation_step( 'group-invites' ) ) echo 'custom'; ?>" enctype="multipart/form-data">
			<h2 style="display:inline;"><?php _e( 'Create a Group', 'buddypress' ); ?></h2> &nbsp;<a class="button small radius" href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_groups_root_slug() ); ?>"><?php _e( 'Groups Directory', 'buddypress' ); ?></a>
			...
			<div class="item-body" id="group-create-body">
				...
				<?php if ( bp_is_group_creation_step( 'group-avatar' ) ) : ?>
					...
					<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
						...
						<input type="submit" name="avatar-crop-submit" class="button small radius" id="avatar-crop-submit" value="<?php _e( 'Crop Image', 'buddypress' ); ?>" />
						...
						<div class="main-column">
							...
							<?php if ( bp_group_has_invites() ) : ?>
								...
								<?php while ( bp_group_invites() ) : bp_group_the_invite(); ?>
									...
									<li clas="four" id="<?php bp_group_invite_item_id(); ?>">
				...
				<?php if ( 'crop-image' != bp_get_avatar_admin_step() ) : ?>
					...
					<div class="submit" id="previous-next">
						...
						<?php if ( !bp_is_first_group_creation_step() ) : ?>
							...
							<input type="button" value="<?php _e( 'Back to Previous Step', 'buddypress' ); ?>" class="button radius secondary" id="group-creation-previous" name="previous" onclick="location.href='<?php bp_group_creation_previous_link(); ?>'" />
						...
						<?php if ( !bp_is_last_group_creation_step() && !bp_is_first_group_creation_step() ) : ?>
							...
							<input type="submit" class="button radius" value="<?php _e( 'Next Step', 'buddypress' ); ?>" id="group-creation-next" name="save" />
						...
						<?php if ( bp_is_first_group_creation_step() ) : ?>
							...
							<input type="submit" value="<?php _e( 'Create Group and Continue', 'buddypress' ); ?>" class="button radius" id="group-creation-create" name="save" />
						...
						<?php if ( bp_is_last_group_creation_step() ) : ?>
							...
							<input type="submit" value="<?php _e( 'Finish', 'buddypress' ); ?>" class="button radius" id="group-creation-finish" name="save" />
