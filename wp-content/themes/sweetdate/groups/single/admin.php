...
<form action="<?php bp_group_admin_form_action(); ?>" name="group-settings-form" id="group-settings-form" class="<?php if ( bp_is_group_admin_screen( 'manage-members' ) ) : ?> admin-members-form <?php endif;?>standard-form <?php if (! bp_is_group_admin_screen( 'delete-group' ) ) echo 'custom'; ?>" method="post" enctype="multipart/form-data" role="main">
...
<?php if ( bp_is_group_admin_screen( 'edit-details' ) ) : ?>
	...
	<p><input type="submit" value="<?php _e( 'Save Changes', 'buddypress' ); ?>" class="button radius" id="save" name="save" /></p>
...
<?php if ( bp_is_group_admin_screen( 'group-avatar' ) ) : ?>
	...
	<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
			...
			<p>
				...
				<input type="submit" name="upload" id="upload" class="button tiny radius" value="<?php _e( 'Upload Image', 'buddypress' ); ?>" />
	...
	<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
		...
		<input type="submit" name="avatar-crop-submit" id="avatar-crop-submit" class="button radius" value="<?php _e( 'Crop Image', 'buddypress' ); ?>" />
		...
		<ul id="admins-list" class="item-list single-line">
			...
			<li class="four columns">
				...
				<h5>
					...
					<span class="small">
						<a class="button tiny radius confirm admin-demote-to-member" href="<?php bp_group_member_demote_link( bp_get_member_user_id() ); ?>"><?php _e( 'Demote to Member', 'buddypress' ); ?></a>
					</span>			
					<?php endif; ?>
				</h5>		
			</li>
				...
				<ul id="mods-list" class="item-list single-line">
					...
					<li>
						...
						<h5>
							<a href="<?php bp_member_permalink(); ?>"> <?php bp_member_name(); ?></a>
							<span class="small">
								<a href="<?php bp_group_member_promote_admin_link( array( 'user_id' => bp_get_member_user_id() ) ); ?>" class="button tiny radius confirm mod-promote-to-admin" title="<?php _e( 'Promote to Admin', 'buddypress' ); ?>"><?php _e( 'Promote to Admin', 'buddypress' ); ?></a>
								<a class="button tiny radius confirm mod-demote-to-member" href="<?php bp_group_member_demote_link( bp_get_member_user_id() ); ?>"><?php _e( 'Demote to Member', 'buddypress' ); ?></a>
							</span>		
						</h5>		
					</li>	
					<?php endwhile; ?>			
					...
					<li class="<?php bp_group_member_css_class(); ?>">
						...
						<h5>
							...
							<?php if ( bp_get_group_member_is_banned() ) : ?>
								...
								<a href="<?php bp_group_member_unban_link(); ?>" class="button tiny radius confirm member-unban" title="<?php _e( 'Unban this member', 'buddypress' ); ?>"><?php _e( 'Remove Ban', 'buddypress' ); ?></a>

							<?php else : ?>

								<a href="<?php bp_group_member_ban_link(); ?>" class="button tiny radius confirm member-ban" title="<?php _e( 'Kick and ban this member', 'buddypress' ); ?>"><?php _e( 'Kick &amp; Ban', 'buddypress' ); ?></a>
								<a href="<?php bp_group_member_promote_mod_link(); ?>" class="button tiny radius confirm member-promote-to-mod" title="<?php _e( 'Promote to Mod', 'buddypress' ); ?>"><?php _e( 'Promote to Mod', 'buddypress' ); ?></a>
								<a href="<?php bp_group_member_promote_admin_link(); ?>" class="button tiny radius confirm member-promote-to-admin" title="<?php _e( 'Promote to Admin', 'buddypress' ); ?>"><?php _e( 'Promote to Admin', 'buddypress' ); ?></a>

							<?php endif; ?>

								<a href="<?php bp_group_member_remove_link(); ?>" class="button tiny radius confirm" title="<?php _e( 'Remove this member', 'buddypress' ); ?>"><?php _e( 'Remove from group', 'buddypress' ); ?></a>
...
<?php if ( bp_is_group_admin_screen( 'delete-group' ) ) : ?>
	...
	<div id="message" data-alert class="alert-box alert">
		<?php _e( 'WARNING: Deleting this group will completely remove ALL content associated with it. There is no way back, please be careful with this option.', 'buddypress' ); ?>
	</div>

	<label><input type="checkbox" name="delete-group-understand" class="button radius" id="delete-group-understand" value="1" onclick="if(this.checked) { document.getElementById('delete-group-button').disabled = ''; } else { document.getElementById('delete-group-button').disabled = 'disabled'; }" /> <?php _e( 'I understand the consequences of deleting this group.', 'buddypress' ); ?></label>

	<?php do_action( 'bp_after_group_delete_admin' ); ?>

	<div class="submit">
		<input type="submit" disabled="disabled" value="<?php _e( 'Delete Group', 'buddypress' ); ?>" class="button radius" id="delete-group-button" name="delete-group-button" />
	</div>
